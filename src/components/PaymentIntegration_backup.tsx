...
const CheckoutForm: React.FC<PaymentIntegrationProps> = ({ selectedPlan, onPaymentComplete }) => {
  ...
  const processStripePayment = async (plan: { price: number; priceId?: string }) => {
    if (!stripe || !elements) {
    ...
    try {
      ...
      // Create payment intent - use priceId if available, otherwise use amount
      const paymentData = plan.priceId 
        ? { priceId: plan.priceId }
        : { amount: plan.price * 100, currency: 'usd' };
