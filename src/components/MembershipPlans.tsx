...
import { Star, Zap, Crown, Shield, CheckCircle } from 'lucide-react';
...
const MembershipPlans = () => {
  ...
  const plans: Plan[] = [
    ...
    },
    {
      ...
      priceId: 'price_1PJd4zSJXqX9XJX9XJX9XJXJ', // Replace with your actual Stripe price ID for $499 plan
      features: [
  ...
  const handlePlanSelect = (plan: Plan) => {
    ...
    // Navigate to payment with the selected plan
    navigate('/payment', {
      state: { 
        selectedPlan: plan
      }
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-950 via-gray-900 to-gray-950 text-white py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-12">
          ...
          <p className="text-xl text-gray-400">Select the plan that fits your trading needs</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {plans.map((plan) => (
            <div 
              key={plan.id}
              className={`relative rounded-xl border ${plan.color} ${plan.bgColor} p-6 flex flex-col h-full`}
            >
              {plan.popular && (
              ...
              <div className="text-center mb-8">
                ...
                <button
                  ...
                  className={`w-full py-3 px-6 rounded-lg font-medium text-white ${plan.buttonColor} transition-colors`}
                >
                  {plan.isAffiliate ? 'Get Started' : 'Choose Plan'}
                </button>
              </div>
